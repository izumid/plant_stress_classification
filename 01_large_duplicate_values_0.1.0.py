...
# MARK: Run Models
def run_train_test(path_base,list_window,path_destination,list_classifier,random_state,verbose,filename):
	int_verbose = int(verbose)
	scaler = MinMaxScaler()
	if not os.path.exists(path_destination): os.makedirs(path_destination)
	total_rounds = len(list_window)*len(list_classifier)
	rounds = 0
	header_txt = ["model","window","samples_summarized","accuracy_train","accuracy_test","presicion","recall","f1_score"]
	filename = filename + "_train_test_split_"

	list_result = []

	for MxN in list_window:
		folder_window = str(MxN[0])+'x'+str(MxN[1])
		path_txt = os.path.join(path_destination,f"{filename}{MxN[0]}x{MxN[1]}.txt")
		...
		for model in list_classifier:
			...
			print(f"Model: {model}({folder_window}). Cross validation fold[{train_test_count}] ({(rounds/total_rounds)*100:.2f}%)")
...
def main():
	...
	config = read_config(os.path.join(os.getcwd(),"01_config_ldv"+".json"))
	...
	if config["classify"]:
		...
		result_feather_read(path_destination=path_result,filename=str(Path(os.path.realpath(__file__)).stem)+"_train_test_split_")
...
if __name__ == "__main__":
	try: 
		if int(input("Type 1 to show windows list: ")):
			config = read_config(os.path.join(os.getcwd(),"01_config_ldv"+".json"))
			#list_window = new_window_size(total_sample_size=config["sample_size"])
			#list_window = window_size(sample_size=config["sample_size"],n=100,min_sample=200,test_combination=100)
			list_window = config["list_window"]
			print(list_window)
