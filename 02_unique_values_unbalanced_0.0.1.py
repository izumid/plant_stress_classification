...
def new_window_size(total_sample_size,start=1,step=1):
	list_window = []
	...
	print(f"list_window: {list_window}")
	#print(sum(test),len(list_window))
	#print(int(list_window[0][0] * list_window[0][1] / 6))
	return(list_window)
...
# MARK: Run Models
def run_train_test(path_base,path_destination,list_classifier,random_state,verbose,k_fold_split,csv=False,filename="result",residual=False):
	int_verbose = int(verbose)
	list_result = []
	list_residual = []
	scaler = MinMaxScaler()

	if k_fold_split == 0: filename = filename + "_train_test_split"
	else: filename = filename + "_stratified_kfold"

	if k_fold_split > 0:
		for folder_window in sorted(os.listdir(path_base)):
			...
			for i,char in enumerate(folder_window):
				if char == 'x': count_char = i

			window = int(folder_window[:count_char])
			samples_summarized = int(folder_window[count_char+1:])
			...
			if model == "DT":
				if folder_window == "100x968": #"4400x22":
					node_depths = classifier.tree_.max_depth
					plt.figure(figsize=(25,10))
					plot_tree(classifier,feature_names=df_train.columns.to_list(), class_names=['0','1'],filled=True,rounded=True,fontsize=14)
					plt.title(f"Decision Tree (Max Depth: {node_depths})")
					plt.savefig("dt.png")

	else:
		for folder_window in sorted(os.listdir(path_base)):
			df_train = pd.read_feather(os.path.join(path_base,folder_window,"dataframe_train.feather"))
			X_train = df_train.iloc[:, :-1]
			y_train = df_train.iloc[:, -1]

			df_test = pd.read_feather(os.path.join(path_base,folder_window,"dataframe_test.feather"))
			...
			for model in list_classifier:
				match model:
					case "DUM": classifier = DummyClassifier(random_state=random_state,strategy="stratified")
					#case "DT": classifier = DecisionTreeClassifier(random_state=random_state,criterion="entropy",class_weight="balanced")
					case "DT": classifier = DecisionTreeClassifier(random_state=random_state,criterion="gini",ccp_alpha=20)
					case "NB": classifier = GaussianNB(priors=None, var_smoothing=1e-09)
					case "KNN": classifier = KNeighborsClassifier(n_neighbors=5,weights='uniform', algorithm='auto', leaf_size=30, p=2, metric='minkowski', metric_params=None, n_jobs=None)
					case "XGB": classifier = xgb.XGBClassifier(random_state=random_state,verbosity=int_verbose,objective="binary:logistic")
					#case "RF": classifier = RandomForestClassifier(random_state=random_state,verbose=int_verbose,criterion="entropy",class_weight="balanced")
					case "RF": classifier = RandomForestClassifier(random_state=random_state,verbose=int_verbose,criterion="gini")
					case "MLP": classifier = MLPClassifier(random_state=random_state,verbose=verbose,solver="adam",activation="logistic",max_iter=1000,hidden_layer_sizes=(100, 100))
					case "SVM": classifier = svm.SVC(random_state=random_state,verbose=verbose,probability=False,C=1.0, kernel='rbf',degree=3,gamma='scale',coef0=0.0,shrinking=True,tol=0.001,cache_size=200,class_weight=None,max_iter=-1,decision_function_shape='ovr', break_ties=False)
				...
				if residual: list_residual.append([str(y_test) ,str(y_predicted)])
	...
	if residual:
		# Must fix this...
		dict_residual = {"y_test": "int" ,"y_predicted": "int"}
		df_residual = pd.DataFrame({col: pd.Series(dtype=dtype) for col, dtype in dict_residual.items()})
		df_residual = pd.DataFrame(list_residual,columns=df_residual.columns.tolist())
		df_residual.to_csv(os.path.join(path_destination,"residual.csv"),sep=';',quotechar='"',encoding="utf-8-sig")
		#if csv: df.to_csv(os.path.join(path_destination,"residual.csv"),sep=';',quotechar='"',encoding="utf-8-sig")
		#else: df.to_feather(os.path.join(path_destination,"residual.feather"))
...
def main():
	...
	config = read_config(os.path.join(os.getcwd(),"02_config_uvu"+".json"))
	...
	dict_sample = {
		"cold_after": 3648,
		"cold_before": 3648,
		"low_light_after": 10971,
		"low_light_before":	10971,
		"manitol_after": 33781,
		"manitol_before": 33781
	}

	dict_futuretest_sample = {
		"cold_after": 16140,
		"cold_before": 3648,
		"low_light_after": 16132,
		"low_light_before":	10971,
		"manitol_after": 16132,
		"manitol_before": 33777
	}

	total_sample_size = sum(dict_sample.values())

	list_window = new_window_size(total_sample_size = total_sample_size)
	# used process
	if config["pre_processing_data"]:
		...
		# -- Unify & Distinct --
		# Unify each stimili into a single file with unique values
		...
		for folder in os.listdir(path_parent_root):
			if "lectrodes" not in folder:
				...
				np_array = np.unique(np.concatenate(np_array))
		...
		for filename in sorted(os.listdir(path_unified_resized)):
			...
			sample_size = dict_sample[name_stimulus]
...
if __name__ == "__main__":
	#print(window_size(sample_size=21888,n=100,min_sample=5,test_combination=10000000))
	#print(window_size(sample_size=43884,n=200,min_sample=10,test_combination=1000))
	new_window_size(total_sample_size=96800)
	#main()